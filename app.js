// NorrisCAM — DXF → Step-reveal toolpath + USB .DAT (G-code)
// Behavior (per your spec):
// - Import DXF: geometry appears immediately
// - Build: generates toolpath + NC, but DOES NOT draw toolpath yet
// - Press S: reveal next motion segment (CUT=red solid, RAPID=blue faint dashed)
// - Press B: go back one segment (removes last revealed segment)
// - Canvas view never auto-changes during build/step (no auto-fit/pan/zoom changes)
// - NC pane scrolls/highlights matching line on every step/back
// - Export prompts file name, saves .dat
// - Supports LINE, (LW)POLYLINE, POLYLINE, SPLINE, ARC, CIRCLE (approximated)
...
// preview segments = what the DXF shows (cyan)
let previewSegs = []; // [{a:{x,y}, b:{x,y}}]

// playback segments = motion segments after building toolpath
// [{a,b,mode:"CUT"|"RAPID", moveIdx, ncLineIdx}]
let playback = [];
...
// reveal stepping
let revealCount = 0; // number of playback segments revealed

// show rapids as faint dashed AFTER step lands
const SHOW_RAPIDS = true;

// view control
const view = { scale: 60, ox: 0, oy: 0, dragging: false, lx: 0, ly: 0 };
let viewTouched = false; // once user pans/zooms, never auto-fit on import again
...
// arc tessellation chord-based
function tessellateArc(center, r, a0, a1, outPrec) {
  // ensure we move shortest direction consistent with DXF arc direction (usually CCW from startAngle to endAngle)
  // We'll handle wrap by moving CCW if a1 < a0
  let start = a0, end = a1;
  if (end < start) end += 2 * Math.PI;

  // step angle based on chord error approx: error ≈ r*(1-cos(dθ/2)) <= outPrec
  // => dθ <= 2*acos(1 - outPrec/r)
  let dTheta = Math.PI / 18; // fallback 10°
  if (r > 1e-9) {
    const x = Math.max(-1, Math.min(1, 1 - (outPrec / r)));
    const maxStep = 2 * Math.acos(x);
    if (isFinite(maxStep) && maxStep > 1e-6) dTheta = Math.min(maxStep, Math.PI / 6); // cap at 30°
  }
...
// build line segments from point list
...
function extractSegments(dxf, outPrec) {
  ...
  for (const e of ents) {
    ...
    // SPLINE (approx using fit/control points)
    if (e.type === "SPLINE") {
    ...
    if (e.type === "ARC") {
      ...
      if (c && isFinite(r) && r > 0) {
        ...
        // dxf-parser often returns degrees; detect if looks like degrees
...
// merge collinear consecutive segments in a chain
...
// chain lines into paths using angle tolerance
function buildPathsFromLines(lines, snapGrid, angleTolDeg, outPrec) {
  ...
  const L = lines
    ...
    .filter(s => isFinite(s.a.x) && isFinite(s.a.y) && isFinite(s.b.x) && isFinite(s.b.y) && dist(s.a, s.b) >= minLen);
...
// order paths with nearest-neighbor (minimize rapids)
...
// merge paths if endpoint is within chainTol of the next start (tool stays down)
...
// Build motion "moves" for multiple depth passes
// Move types: retract, rapidXY, plunge, feedXY, cutLine
function buildMoves(paths, opts) {
  ...
  for (const z of passZ) {
    ...
    for (const p of paths) {
      ...
      // only retract+rapid if not continuous to start
      if (!samePt(curXY, start, opts.chainTol)) {
        ...
        // already at start; just ensure at depth
        if (curZ !== z) {
          plunge(z);
        }
        setFeedXY();
      ...
      // keep tool down between paths if merged; otherwise next loop retracts/rapids as needed
    }
    retract();
  }
...
// NC generation + mapping moveIdx => NC line index
function buildNC(moves, opts) {
  const P = opts.outPrec;
  const lines = [];
  const moveToLine = new Map();

  function push(line, moveIdx = null) {
...
// Build playback segments with per-segment mapping to NC line
...
function drawRevealedToolpath() {
  ...
  if (SHOW_RAPIDS) {
    ctx.save();
    ctx.strokeStyle = "rgba(122,167,255,0.35)"; // faint
    ctx.lineWidth = 2;
    ctx.setLineDash([6, 6]);
    for (let i = 0; i < n; i++) {
      const s = playback[i];
...
function draw() {
  ...
  // always show DXF geometry (cyan) as the "model"
  drawPreviewGeometry();

  // toolpath only reveals as you step
  if (playback.length) {
...
// disable scrub permanently (not used)
$("scrub").disabled = true;
...
$("file").addEventListener("change", async (e) => {
  ...
  if (!segs.length) {
    setStatus("bad", "No supported geometry", "Need LINE/POLYLINE/SPLINE/ARC/CIRCLE.");
    alert("No usable geometry found.\nTry: explode/convert splines, export R12/ASCII.");
    return;
  }
  ...
  // Optional: only auto-fit on very first import if user hasn't touched view
  // (keeps "screen never changes" once teacher pans/zooms)
  if (!viewTouched) {
...
$("buildToolpath").addEventListener("click", () => {
  ...
  try {
    ...
    // Build playback segments + attach NC line index mapping
    playback = buildPlayback(moves, moveToLine);

    // Reset stepping: nothing revealed until S is pressed
    revealCount = 0;
    ...
    // IMPORTANT: do NOT change view on build (per your spec)
    setStatus("good", "Built", `Paths: ${paths.length} | Moves: ${playback.length} | Press S to reveal.`);
    draw();
  } catch (err) {
    console.error(err);
...
// ---------- Origin changes update preview only (no view changes) ----------
$("origin").addEventListener("change", () => {
  ...
  // clear toolpath + nc (must rebuild)
  playback = [];
  moves = [];
  ncLines = [];
  lastNC = "";
  ...
  // no auto-fit here; keep screen stable
